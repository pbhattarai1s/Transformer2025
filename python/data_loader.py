...
class InclusiveDataset(Dataset):
    ...
    def __init__(self, h5_file, norm_file):
        self.h5_file = h5py.File(h5_file, 'r')
        self.events, self.objects = self.h5_file['events'], self.h5_file['objects']
        ...
        self.event_means = np.array([norm_params['events'][name]['mean'] for name in self.events.dtype.names if name != 'label'])
        self.event_stds = np.array([norm_params['events'][name]['std'] for name in self.events.dtype.names if name != 'label'])
    ...
    def __getitem__(self, idx):
        ...
        event_features = np.array([event[name] for name in self.events.dtype.names if name != 'label'])
